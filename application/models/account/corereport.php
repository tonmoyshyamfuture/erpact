...
class corereport extends CI_Model {
...
//        return $query->result_array();
    ...
    public function getledgerDetailsByIdByDate($ledger_id, $from_date, $to_date) {
        $from_date=$from_date.' 00:00:00';
        $to_date=$to_date.' 23:59:59';
        $this->db->select('ET.type,E.entry_no,E.id,E.ledger_ids_by_accounts,E.create_date,LD.ladger_id,L.ladger_name,L.account_type,L.ledger_code,L.opening_balance,LD.account,LD.balance,LD.modified_date');
        $this->db->from('ladger_account_detail AS LD');
        $this->db->join('ladger AS L', 'L.id = LD.ladger_id');
        $this->db->join('entry AS E', 'E.id = LD.entry_id');
        $this->db->join('entry_type AS ET', 'ET.id = E.entry_type_id');
...
//    public function getAllLedgerByGroupIdByDate($group_id = NULL, $from_date, $to_date) {
//        $from_date=$from_date.' 00:00:00';
//        $to_date=$to_date.' 23:59:59';
...
//    }
    ...
    //get all ledger by group id and date range 13/01/2018
