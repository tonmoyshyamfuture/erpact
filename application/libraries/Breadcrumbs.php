        ...
        if ($this->breadcrumbs) {
            ...
            foreach ($this->breadcrumbs as $key => $crumb) {
                ...
                if (end($keys) == $key) {
                    ...
                    $withoutExt = preg_replace('/\\.[^.\\s]{3,4}$/', '', $crumb['href']);  
                     $output .= $this->crumb_open . '<a href="' . $withoutExt . '">' . $crumb['page'] . '</a> ' . $this->crumb_divider . $this->crumb_close;
                    }else if(preg_match($re, $crumb['href'])){
                     $withoutExt = preg_replace('/\\.[^.\\s]{3,4}$/', '', $crumb['href']);
                      $output .= $this->crumb_open . '<a href="' . $withoutExt . '">' . $crumb['page'] . '</a> ' . $this->crumb_divider . $this->crumb_close;
                    }else{
            ...
            // return output
            echo $output . $this->tag_close . PHP_EOL;
        }
